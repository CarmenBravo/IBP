...
# Create tabel containing info on condition and library type per count file
samples = list.files(paste0(wd,"/HTSeqCount_files"))
samples = sub("~?/?(.+/)*","",samples)
condition = sub("\\d+\\.txt","",samples,perl=T)
